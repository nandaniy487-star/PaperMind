...
export default function Methodology({ onBack, onNavigate }) {
  ...
  const fetchMethodology = async () => {
    ...
    try {
      const data = await generateMethodology(paperId);

      if (data.methodology) {
        setMethodology(data.methodology);
      } else {
        setError(data.message || 'No methodology breakdown could be generated for this paper.');
      }
    } catch (err) {
      console.error('Error generating methodology analysis:', err);
      setError(
        'Sorry, we could not generate the methodology analysis. Please make sure your research paper has been uploaded and the backend is running.'
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="study-notes-container">
        ...
        <div className="study-notes-header">
          <div className="upload-badge">
            ...
            <span>Methodology Inspector</span>
          </div>

          <h1 className="study-notes-title">
            Methodology <span className="gradient-text">Analysis</span>
          </h1>

          <p className="study-notes-subtitle">
            Understand how the research was designed, algorithms evaluated, and empirical experiments structured.
          </p>
        </div>

        {/* Workspace Quick-Navigation Tabs */}
        <WorkspaceTabs activeTab="methodology" onNavigate={onNavigate} />
          ...
          <div className="active-doc-right">
            {methodology && !isLoading && (
              <button
                ...
                onClick={handleCopy}
                title="Copy methodology analysis to clipboard"
              >
                {copied ? (
                  <>
        ...
        <div className="research-analysis-card">
          {isLoading ? (
            <div className="analysis-state-box">
              ...
              <h3 className="state-title">Analyzing Methodology...</h3>
              <p className="state-description">
                Parsing research methods, experimental variables, model architectures, and validation setups.
              </p>
            </div>
          ) : error ? (
            <div className="analysis-state-box error-state">
              ...
              <h3 className="state-title">Unable to Generate Methodology</h3>
              <p className="state-description">{error}</p>
              <button type="button" className="btn-secondary" onClick={fetchMethodology}>
                Try Again
              </button>
            </div>
          ) : (
